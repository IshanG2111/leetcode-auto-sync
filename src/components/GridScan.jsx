...
export const GridScan = ({
  ...
  useEffect(() => {
    ...
    const renderer = new THREE.WebGLRenderer({ antialias: !isLowEnd, alpha: true, powerPreference: "high-performance" });
    ...
    let composer = null;
    const shouldPost = enablePost && !isLowEnd;
    if (shouldPost) {
    ...
    const tick = () => {
      ...
      // Monitor FPS for low performance, handling tab visibility & stutters
      if (!isLowEnd) {
        if (dt > 0.25) {
          // Reset count on background tab wake or massive stutter to avoid false downgrade
          frameCount = 0;
          lastFpsUpdateTime = now;
        } else {
          frameCount++;
          if (now - lastFpsUpdateTime > 2000) { // check every 2 seconds
            const fps = (frameCount * 1000) / (now - lastFpsUpdateTime);
            if (fps < 38) {
              console.warn(`[GridScan] Low performance detected (${Math.round(fps)} FPS). Downgrading quality.`);
              setIsLowEnd(true);
            }
            frameCount = 0;
            lastFpsUpdateTime = now;
